    ...
    <div class="container mt-2 p-2 m-0">
        <form id="frmLogin">
            <div class="passport bg-white">
                ...
                <a id="btnSubmit"
                   class="btn btn-success  btn-block mt-4 text-white pt-3 pb-3">登&nbsp;&nbsp;&nbsp;&nbsp;录</a>
            </div>
        </form>
...
<script>
    function showTips(isShow, css, text) {
        if (isShow) {
            $("#tips").removeClass("d-none").hide().addClass(css).text(text).fadeIn(200);

        } else {
            $("#tips").text("").fadeOut(200).removeClass().addClass("alert")
        }
    }
